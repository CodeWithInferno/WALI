...
export function mapDatabaseError(message: string): EdgeError {
  const mappings: ReadonlyArray<readonly [string, string, number, boolean]> = [
    ...
    ["WALI_DELETION_LEASE_INVALID", "stale_revision", 409, true],
    [
      "WALI_DELETION_AUTOMATION_UNAVAILABLE",
      "temporarily_unavailable",
      503,
      true,
    ],
    [
      "WALI_DELETION_AUTOMATIC_RECOVERY_REQUIRED",
      "automatic_recovery_required",
      409,
      false,
    ],
    ["WALI_ARTIFACT_DELETION_FENCED", "release_not_available", 409, false],
