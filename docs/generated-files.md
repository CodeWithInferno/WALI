...
- `project.yml` (direct) and `project-store.yml` (Store), with shared target templates
  in `project-common.yml`, for targets, settings, dependencies, and bundle layout;
...
- `WALI.xcodeproj/` and `WALIStore.xcodeproj/` are generated by XcodeGen.
