...
- Live read-only revalidation on build `25G83` found the same manifest version, category and asset field sets, global linked-node shape, and empty `Displays` and `Spaces` maps, so it remains in compatibility epoch 1 revision 1.
...
- Live repeat-lock validation on build `25G83` found that Apple's Aerial timebase continued beyond an 11.08-second WALI asset through at least 176 seconds with zero queued frames, producing black output while the WALI selection and files remained valid. Restarting WallpaperAgent and its Aerial extension reopened the same asset at time zero without a store write.
- ADR 0010 therefore adds one read-only, ownership-gated Apple wallpaper-process refresh for each distinct session-lock transition. Duplicate workspace/distributed lock notifications are coalesced; ordinary reconciliation remains mutation-free and does not restart playback.

- ADR 0008 establishes the safety boundary, ADR 0009 supplies revision-1 global linked activation, and ADR 0010 supplies repeat-lock playback restart for exact system builds `25F80` and `25G83`, Aerial manifest version 1, and provider `com.apple.wallpaper.choice.aerials`.
