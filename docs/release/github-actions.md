...
A local-preview candidate cannot be published through this workflow. Source
implementation or a valid manifest does not establish production readiness.
...
| `WALI_APP_PROFILE_BASE64` | `release-signing` | Existing macOS Developer ID profile for `io.github.codewithinferno.wali.WALI`, authorizing `group.com.wali.shared`; the direct email-code flow requires no native Sign in with Apple entitlement. |
...
Commit the reviewed `Config/Marketplace.production.json` described in
[production configuration](production-configuration.md), then run
`ruby scripts/production-config.rb json .`. Set `WALI_MARKETPLACE_CONFIG_JSON` to
that output. It contains **exactly** these fourteen string fields; the adapter
requires byte-for-byte agreement with the committed manifest and its digest:

| Field | Required value |
| --- | --- |
| `WALI_MARKETPLACE_ENABLED` | `YES` |
| `WALI_RELEASE_MODE` | `production` |
...
| `WALI_SUPABASE_PUBLISHABLE_KEY_SHA256` | SHA256 of the exact publishable-key bytes. |
...
| `WALI_PRODUCTION_CONFIGURATION_SHA256` | Derived canonical manifest digest, generated by the command above. |
...
setup. It generates only the ignored production xcconfig and checks all fourteen
values in the actual archived app, plus the agent's corresponding trust fields.
The candidate's app digest includes those
...
2. In Actions, select **Signed GitHub release**, **Run workflow**, branch `main`,
   ...
   `gh workflow run release.yml --ref main --field tag=v0.1.0-beta.1`, using the actual
