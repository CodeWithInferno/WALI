...
A pre-still server may omit this field. Migration012 derives the advertisement directly from the existing `still_uploads_enabled` gate: false returns MP4/QuickTime; true additionally returns JPEG/PNG. It preserves the existing authenticated function identity, grants, metadata and account guard. The migration does not enable still intake; JPEG/PNG advertisement follows the accepted worker/signing/V2 rollout.
