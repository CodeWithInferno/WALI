...
struct AgentWallpaperMediaContentTests {
    @Test("Video and still retain distinct exact wire shapes")
    func roundTrip() throws {
        let image = AgentWallpaperMediaContent.still(imageURL: URL(fileURLWithPath: "/private/image.png"))
        let video = AgentWallpaperMediaContent.video(masterURL: URL(fileURLWithPath: "/private/master.mp4"), previewURL: URL(fileURLWithPath: "/private/preview.mp4"), duration: 12.5)
        for value in [image, video] {
            let data = try JSONEncoder().encode(value)
            #expect(try JSONDecoder().decode(AgentWallpaperMediaContent.self, from: data) == value)
    ...
    func rejectsInvalid(json: String) {
        #expect(throws: (any Error).self) { try JSONDecoder().decode(AgentWallpaperMediaContent.self, from: Data(json.utf8)) }
    }

    @Test("Encoding also rejects invalid direct construction")
    func invalidEncoding() {
        #expect(throws: (any Error).self) {
            try JSONEncoder().encode(AgentWallpaperMediaContent.video(masterURL: URL(fileURLWithPath: "/a"), previewURL: URL(fileURLWithPath: "/b"), duration: .infinity))
        }
    }
}
